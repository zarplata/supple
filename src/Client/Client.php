...
class Client implements ClientInterface
{
    ...
    public function __construct(ElasticsearchClient $client, bool $hasMappingType)
    {
        $this->client = $client;
        $this->hasMappingType = $hasMappingType;
    }
    ...
    public function batch(array $changeSets): void
    {
        ...
        foreach ($response['items'] as $i => $item) {
            ...
            if (isset($result['error'])) {
                $ordered[$i]->addError(
                    IndexationError::create($action, $result['_index'], $result['_id'], $result['error'])
                );
            }
        }
    }
